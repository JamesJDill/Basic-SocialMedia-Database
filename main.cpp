...
class FlatFile {
    private:
        ...
        std::string posts_file_path;
        std::string engagements_file_path;

        std::mutex flatfile_mutex;

    public:
        FlatFile(std::string users_csv_path, std::string posts_csv_path, std::string engagements_csv_path){
            users_file_path = users_csv_path;
            posts_file_path = posts_csv_path;
            engagements_file_path = engagements_csv_path;
        }
        ...
        void loadFlatFile() {
            ...
            std::ifstream posts_file(posts_file_path); 
            std::ifstream engagements_file(engagements_file_path);
            ...
            if (!users_file) {
                ...
                while(std::getline(users_file, line)) {
                    ...
                    std::getline(row, id_str, ',');
                    std::getline(row, username, ',');
                    std::getline(row, location);
            ...
            // Load Posts File
            if (!posts_file) {
                std::cerr << "Unable to open posts file" << std::endl;
            } else {
                // Skip column labels
                ...
                while(std::getline(posts_file, line)) {
                    ...
                    std::istringstream row(line);
                    std::getline(row, id_str, ',');
                    std::getline(row, content, ',');
                    std::getline(row, username, ',');
                    std::getline(row, views_str);
            ...
            // Load Engagements File
            if (!engagements_file) {
                std::cerr << "Unable to open engagements file" << std::endl;
            } else {
                // Skip column labels
                std::getline(engagements_file, line);
                while(std::getline(engagements_file, line)) {
                    std::string id_str;
                    std::string postid_str;
                    std::string username;
                    std::string type;
                    std::string comment;
                    ...
                    std::getline(row, id_str, ',');
                    std::getline(row, postid_str, ',');
                    std::getline(row, username, ',');
                    std::getline(row, type, ',');
                    std::getline(row, comment, ',');
                    std::getline(row, timestamp_str);

                    int id = std::stoi(id_str);
                    int postid = std::stoi(postid_str);
                    int timestamp = std::stoi(timestamp_str);
                    engagements[id] = std::make_unique<Engagement>(id, postid, username, type, comment, timestamp);
                }
            }
        }
        ...
        void loadFlatFile_MultiThread() {
            // Lambda thread function for loading users
            auto thread_load_users = [this]() {
                std::vector<std::unique_ptr<User>> user_list;
                std::ifstream users_file(users_file_path);
                std::string line;
                ...
                while(std::getline(users_file, line)) {
                    ...
                    int id = std::stoi(id_str);
                    user_list.push_back(std::make_unique<User>(id, username, location));
                }
                // Update users map
                std::lock_guard<std::mutex> lock(flatfile_mutex);
                for (auto& user : user_list) {
                    users[user->id] = std::move(user);
                }
            };
            // Lambda thread function for loading posts
            auto thread_load_posts = [this]() {
                std::vector<std::unique_ptr<Post>> post_list;
                std::ifstream posts_file(posts_file_path); 
                std::string line;

                // Skip column labels
                std::getline(posts_file, line);
                // Read in posts csv data
                while(std::getline(posts_file, line)) {
                    std::string id_str;
                    std::string content;
                    std::string username;
                    ...
                    int id = std::stoi(id_str);
                    int views = std::stoi(views_str);
                    post_list.push_back(std::make_unique<Post>(id, content, username, views));
                }
                // Update posts map
                std::lock_guard<std::mutex> lock(flatfile_mutex);
            ...
            // Lambda thread function for loading engagements
            auto thread_load_engagements = [this]() {
                std::vector<std::unique_ptr<Engagement>> engagement_list;
                std::ifstream engagements_file(engagements_file_path);
                std::string line;

                // Skip column labels
                std::getline(engagements_file, line);
                // Read in engagements csv data
                while(std::getline(engagements_file, line)) {
                    std::string id_str;
                    std::string postid_str;
                    std::string username;
                    ...
                    int postid = std::stoi(postid_str);
                    int timestamp = std::stoi(timestamp_str);        
                    engagement_list.push_back(std::make_unique<Engagement>(id, postid, username, type, comment, timestamp));
                }
                // Update Engagements map
                std::lock_guard<std::mutex> lock(flatfile_mutex);
                for (auto& engagement : engagement_list) {
                    engagements[engagement->id] = std::move(engagement);
                }
            };
            // Creates and starts threads
            std::vector<std::thread> threads;
            threads.emplace_back(thread_load_users);
            threads.emplace_back(thread_load_posts);
            threads.emplace_back(thread_load_engagements);
        ...
        bool incrementPostViews(int post_id, int views_count) {
            std::lock_guard<std::mutex> lock(flatfile_mutex);
            auto post = posts.find(post_id);
            if (post != posts.end()) {
                post->second->views += views_count;

                // Overwrite posts.csv
                std::ofstream posts_file(posts_file_path, std::ios::out | std::ios::trunc);
                for (const auto& [id, post] : posts) {
                    posts_file << post->toCSV();
                }
                return true;
            }
            return false;
        ...
        void addEngagementRecord(Engagement& record) {
            ...
            // Overwrite engagements.csv
            std::ofstream engagements_file(engagements_file_path, std::ios::out | std::ios::trunc);
            for (const auto& [id, engagement] : engagements) {
                engagements_file << engagement->toCSV();
            }
        }
        ...
        std::vector<std::pair<int,std::string> > getAllUserComments(int user_id) {
            ...
            for (const auto& [id, engagement] : engagements) {
                if (engagement->username == username && engagement->type == "comment") {
                    list.emplace_back(engagement->postId, engagement->comment);
                }
            }
        ...
        std::pair<int,int> getAllEngagementsByLocation(std::string location) {
            ...
            for (const auto& user : users) {
                if (location == user.second->location) {
                    for (const auto& engagement : engagements) {
                        if (user.second->username == engagement.second->username) {
                            if (engagement.second->type == "comment") {
                                counts.second += 1;
                            } 
                            else if (engagement.second->type == "like") {
                                counts.first += 1;
                            }
                        }
                    }
        ...
        bool updateUserName(int user_id, std::string new_username) {   
            ...
            std::string old_username = user_it->second->username;
            user_it->second->username = new_username;
            flatfile_mutex.unlock();

            std::lock_guard<std::mutex> lock(flatfile_mutex);
            for (auto& post : posts) {
                if (post.second->username == old_username) {
                    post.second->username = new_username;
                }
            }

            for (auto& engagement : engagements) {
                if (engagement.second->username == old_username) {
                    engagement.second->username = new_username;
                }
            }

            // Overwrite users.csv
            std::ofstream users_file(users_file_path, std::ios::out | std::ios::trunc);
            for (const auto& [id, user] : users) {
                users_file << user->toCSV();
            }
